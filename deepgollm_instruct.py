...
LLM_PATH = {
    'llama3': '/gemini-1/space/ckpts/zhangluhao/Meta-Llama-3___1-8B-Instruct/',
    'qwen': '/gemini-1/space/ckpts/wangsq/others/model/qwen2.5/qwen2.5-7B-Instruct/'
}
...
#         self.tokenizer.padding_side = "left"
    ...
    # 🔹 InterPro处理
    # =========================
    def build_ipr_text(self, interpros, ipr_desc_dict):

        ipr_list = []
        for ipr in sorted(interpros):
            if ipr in ipr_desc_dict:
                ipr_list.append(f"{ipr} ({ipr_desc_dict[ipr]})")
            else:
                ipr_list.append(ipr)

        return " ".join(ipr_list)
    ...
    def predict_dataset(
        ...
        for batch_prompts, lables in tqdm(dataloader, desc="Predicting"):
            ...
            for offset, out in enumerate(outputs):
                ...
                results.append({"labels":lables[offset], "preds":','.join(go_terms)})
...
def main(data_root, ont, model_name, batch_size, few_shot, device):
    ...
    # ===== 预测 =====
    preds,results = model.predict_dataset(
        test_instruction,
        terms_dict,
        batch_size=batch_size
    )
    ...
    test_df.to_pickle(f"{data_root}/{ont}/predictions_llm_instruct_shots{few_shot}_{model_name}.pkl")
    with open(f"{data_root}/{ont}/predictions_llm_instruct_shots{few_shot}_{model_name}.txt", "w") as f:
